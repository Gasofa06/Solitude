...
fn deserialize_polymatrix_rng(
  a: &mut PolyMatrixRaw,
  data: &[u8],
  rng: &mut ChaCha20Rng
) -> usize {
  let (first_row, rest) = a.data
    .as_mut_slice()
...
impl<'a> PublicParameters<'a> {
  ...
  fn from_ntt_alloc_vec(
    v: &Vec<PolyMatrixNTT<'a>>
  ...
  pub fn deserialize(params: &'a Params, data: &[u8]) -> Self {
    // `assert_eq!(...)` comprueba si dos expresiones son iguales. Si las expresiones son iguales, no ocurre nada y el programa continua. Si las expresiones son diferentes, sin embargo, el programa finaliza e imprime un mensaje de error.
    ...
    if params.expand_queries {
      let mut v_expansion_left = new_vec_raw(
        params,
        params.g(),
        2,
        params.t_exp_left
      ...
      idx += deserialize_vec_polymatrix_rng(
        &mut v_expansion_left,
        &data[idx..],
        &mut rng
      );

      let mut v_expansion_right = new_vec_raw(
        params,
        params.stop_round() + 1,
        2,
        params.t_exp_right
...
impl<'a> Client<'a> {
  ...
  fn generate_keys_impl(
    ...
    if params.expand_queries {
      ...
      pp.v_expansion_left = Some(
        self.generate_expansion_params(
          params.g(),
          params.t_exp_left,
          &mut rng,
          &mut rng_pub
      ...
      pp.v_conversion = Some(
        Vec::from_iter(once(PolyMatrixNTT::zero(params, 2, 2 * params.t_conv)))
      );
  ...
  pub fn generate_query(&self, idx_target: usize) -> Query<'a> {
    ...
    if params.expand_queries {
      ...
      let inv_2_g_first = invert_uint_mod(
        1 << params.g(),
        params.modulus
      ).unwrap();
      let inv_2_g_rest = invert_uint_mod(
        1 << (params.stop_round() + 1),
        params.modulus
      ).unwrap();
      ...
      if params.db_dim_2 == 0 {
        ...
        for i in 0..params.poly_len {
          sigma.data[i] = multiply_uint_mod(
            sigma.data[i],
            inv_2_g_first,
            params.modulus
          );
        ...
        for i in 0..params.poly_len / 2 {
          sigma.data[2 * i] = multiply_uint_mod(
            sigma.data[2 * i],
            inv_2_g_first,
            params.modulus
          );
          sigma.data[2 * i + 1] = multiply_uint_mod(
            sigma.data[2 * i + 1],
            inv_2_g_rest,
            params.modulus
          );
      ...
      for i in 0..num_expanded {
        ...
        reg_cts.push(
          self.encrypt_matrix_reg(&to_ntt_alloc(&sigma), &mut rng, &mut rng_pub)
        );
      }
      // reorient into server's preferred indexing
      reorient_reg_ciphertexts(
        self.params,
        reg_cts_buf.as_mut_slice(),
        &reg_cts
      );
  ...
  pub fn decode_response(&self, data: &[u8]) -> Vec<u8> {
    ...
    let mut result = PolyMatrixRaw::zero(
      &params,
