...
pub fn log2_ceil_usize(a: usize) -> usize {
  // `f64::ceil` es un método estático proporcionado por el tipo de datos `f64` (número de punto flotante de 64 bits) que redondea un número hacia arriba al entero más cercano.

  // `f64::log2` también es un método estático proporcionado por el tipo de datos `f64` que calcula el logaritmo en base 2 de un número.

  // `(a as f64)` Convierte el parámetro a de tipo `usize` a `f64` para poder aplicar la función `log2`.

  // `as usize` Convierte el resultado de `f64::ceil` nuevamente a `usize`, ya que la función debe devolver un valor de este tipo.
...
pub fn recenter(val: u64, from_modulus: u64, to_modulus: u64) -> u64 {
  ...
  a_val =
    a_val +
    (from_modulus_i64 / to_modulus_i64) * to_modulus_i64 +
    2 * to_modulus_i64;
...
pub fn divide_uint192_inplace(
  mut numerator: [u64; 3],
  denominator: u64
) -> ([u64; 3], [u64; 3]) {
  let mut numerator_bits = get_significant_bit_count(&numerator);
  let mut denominator_bits = get_significant_bit_count(
    slice::from_ref(&denominator)
  );
  ...
  let shifted_denominator = left_shift_uint192(
    shifted_denominator,
    denominator_shift
  );
  ...
  while numerator_bits == denominator_bits {
      ...
      ) != 0
    {
      ...
      add_uint(&difference.clone(), &numerator, uint64_count, &mut difference);
...
pub fn rescale(a: u64, inp_mod: u64, out_mod: u64) -> u64 {
  ...
  let mut result =
    (val + ((sign * (inp_mod_i64 / 2)) as i128)) / (inp_mod as i128);
  result =
    (result + (((inp_mod / out_mod) * out_mod) as i128) + 2 * out_mod_i128) %
    out_mod_i128;
...
mod test {
  ...
  fn divide_uint192_inplace_correct() {
    assert_eq!(divide_uint192_inplace([35, 0, 0], 7), ([0, 0, 0], [5, 0, 0]));
    assert_eq!(
      divide_uint192_inplace([0x10101010, 0x2b2b2b2b, 0xf1f1f1f1], 0x1000),
      ([0x10, 0, 0], [0xb2b0000000010101, 0x1f1000000002b2b2, 0xf1f1f])
    );
  }
  ...
  fn barrett_reduction_u128_raw_correct() {
    ...
    let exec = |val| {
      barrett_reduction_u128_raw(
        66974689739603969u64,
        7906011006380390721u64,
        275u64,
        val
      )
    };
