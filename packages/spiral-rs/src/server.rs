...
pub fn coefficient_expansion(
  ...
  for r in 0..g {
    ...
    let action_expand = |(i, v_i): (usize, &mut PolyMatrixNTT)| {
      if
        (stop_round > 0 && r > stop_round && i % 2 == 1) ||
        (stop_round > 0 &&
          r == stop_round &&
          i % 2 == 1 &&
      ...
      println!("params.t_exp_left: {}", t_exp_left);
      println!("t_exp_right: {}", t_exp_right);

      let mut ginv_ct_left = PolyMatrixRaw::zero(params, t_exp_left, 1);
      let mut ginv_ct_left_ntt = PolyMatrixNTT::zero(params, t_exp_left, 1);
      let mut ginv_ct_right = PolyMatrixRaw::zero(params, t_exp_right, 1);
      let mut ginv_ct_right_ntt = PolyMatrixNTT::zero(params, t_exp_right, 1);

      let (w, _gadget_dim, gi_ct, gi_ct_ntt) = match r != 0 && i % 2 == 0 {
        true =>
          (&v_w_left[r], t_exp_left, &mut ginv_ct_left, &mut ginv_ct_left_ntt),
        false =>
          (
            &v_w_right[r],
            t_exp_right,
    ...
    v[0..num_in].par_iter_mut().enumerate().for_each(action_expand);
    v[num_in..num_out].par_iter_mut().enumerate().for_each(action_expand);
  }
}
...
pub fn regev_to_gsw<'a>(
  ...
  v_gsw
    ...
    .for_each(|(i, ct)| {
      ...
      let mut ginv_c_inp_ntt = PolyMatrixNTT::zero(
        params,
        2 * params.t_conv,
        1
      );
  ...
  for z in 0..params.poly_len {
    ...
    for i in 0..num_per {
      for c in 0..pt_cols {
        ...
        for o_jm in 0..outer_limit {
          unsafe {
            ...
            for i_jm in 0..inner_limit / 4 {
              ...
              sums_out_n0 = _mm256_add_epi64(
                sums_out_n0,
                _mm256_mul_epu32(a_lo, b_lo)
              );
              sums_out_n2 = _mm256_add_epi64(
                sums_out_n2,
            ...
            for idx in 0..4 {
              ...
              sums_out_n0_u64_acc[idx] = barrett_coeff_u64(
                params,
                val + sums_out_n0_u64_acc[idx],
                0
              );
            }
            for idx in 0..4 {
              let val = sums_out_n2_u64[idx];
              sums_out_n2_u64_acc[idx] = barrett_coeff_u64(
                params,
                val + sums_out_n2_u64_acc[idx],
                1
              );
            }
          }
        ...
        out[i].data[idx_c] = barrett_coeff_u64(
          params,
          sums_out_n0_u64_acc[0] + sums_out_n0_u64_acc[2],
          0
        );
        idx_c += pt_cols * crt_count * poly_len;
        out[i].data[idx_c] = barrett_coeff_u64(
          params,
          sums_out_n0_u64_acc[1] + sums_out_n0_u64_acc[3],
          0
        );
  ...
  for z in 0..params.poly_len {
    ...
    for i in 0..num_per {
      for c in 0..pt_cols {
        ...
        let mut n = 0;
        let mut idx_c = c * (crt_count * poly_len) + n * poly_len + z;
        out[i].data[idx_c] = (sums_out_n0_0 %
          (params.moduli[0] as u128)) as u64;
        idx_c += pt_cols * crt_count * poly_len;
        out[i].data[idx_c] = (sums_out_n0_1 %
          (params.moduli[0] as u128)) as u64;

        // output n1
        n = 1;
        idx_c = c * (crt_count * poly_len) + n * poly_len + z;
        out[i].data[idx_c] = (sums_out_n1_0 %
          (params.moduli[1] as u128)) as u64;
        idx_c += pt_cols * crt_count * poly_len;
        out[i].data[idx_c] = (sums_out_n1_1 %
          (params.moduli[1] as u128)) as u64;
      }
    }
  }
...
pub fn generate_random_db_and_get_item<'a>(
  ...
  let mut item = PolyMatrixRaw::zero(
    params,
    params.instances * params.n,
    params.n
  );
...
pub fn load_item_from_seek<'a, T: Seek + Read + Send + Sync>(
  ...
  let bytes_per_chunk = f64::ceil(
    (db_item_size as f64) / (chunks as f64)
  ) as usize;
  let logp = f64::ceil(f64::log2(params.pt_modulus as f64)) as usize;
  let modp_words_per_chunk = f64::ceil(
    ((bytes_per_chunk * 8) as f64) / (logp as f64)
  ) as usize;
  ...
  let modp_words_read = f64::ceil(
    ((bytes_read * 8) as f64) / (logp as f64)
  ) as usize;
...
pub fn load_db_from_seek(params: &Params, fname: &String) -> AlignedMemory64 {
  ...
  // La base de datos (_data base_, _db_) es representada como un hypercubo. Mas concretamente, la _db_ se estructura como una matriz de `V2 + 1` dimensiones, con un tamaño determinado por `2^(V1 x V2)` o `2^V1 x 2 x 2 x ... x 2(subindice: V2)`. En este caso `params.db_dim_1` es el equivalente de `V1` y `params.db_dim_2` representa a `V2`. De la misma forma, `1 << x` es lo mismo que elevar 2 a cualquier valor que se encuntre a la derecha de `<<`.

  // Dicho esto, sabemos que la variable `dim0` hace referencia a la primera dimension de la matriz, que viene dada por el numero de filas que tenga esta (en una matriz de `n x m`, `dim0` equivaldria a `n`); y que`num_per` se refiere a la segunda dimension de la matriz, que viene dada por su numero de columnas (`num_per` equivaldria a `m`)

  // Al multiplicar el numero de filas (`dim0`) por el numero de columnas (`num_per`) de una matriz, se obtiene el nuumero total de elementos de la matriz (o, en este caso, de la _db_). Este valor es representado por la variable `num_items`.
  ...
  // `thread_local!(...)` se utiliza para crear variables que son locales a cada hilo (thread). Esto significa que cada hilo tendrá su propia copia de la variable, con la que podra operar sin interferir en las versiones de los otros hilos.

  // `RefCell<...>`

  thread_local! {
    static STORE: RefCell<Option<File>> = RefCell::new(None);
  }

  // `(0..total_idx).into_par_iter().for_each(|idx| {...}` divide el rango de índices del 0 al `total_idx - 1` en múltiples partes y ejecuta una clausura en paralelo para cada uno de estos indices (`idx`) utilizando la librería `rayon`.

  // En `STORE.with(|cell| {...})` la variable `STORE` es de tipo `RefCell<...>`, entonces, al llamar la funcion `with(...)` en la variable `STORE` y pasar un _closure_ (`|...| {...}`) como argumento, el _closure_ recibe una referencia a la celda completa contenida en `STORE`.

  // Para obtener una referencia mutable al contenido de la celda (si `RefCell<T>` el contenido es `T`) y poder realizar operacion en él (que afecten al valor real contenido en `RefCell`), utilizamos `cell.borrow_mut()`.

  // Destacamos que `local_store` contiene una referencia mutable al contenido de la `RefCell` de `STORE`, en este caso de tipo `Option<File>`. Cuando utilizamos `local_store.is_none()` estamos verificando si el contenido de `RefCell` es `None` (cunado se crea la variable `STORE` se le assigna valor `None`). Si es `None`, entonces se abre un archivo (`File::open(fname).unwrap()`) y se asigna a `local_store` como `Some(File)`. El archivo que se abre es la _db_ aun no procesada.
  ...
  (0..total_idx).into_par_iter().for_each(|idx| {
    STORE.with(|cell| {
      ...
      // Al pasar `indices` con una referencia mutable (`&mut`) la funcion (en este caso `decompose_index`) tiene la capacidad de modificar el valor real de la variable original.
      // `decompose_index` devuelve (...)
      ...
      let mut db_item = load_item_from_seek(
        params,
        &mut file,
        instance,
        trial,
...
pub fn encode(params: &Params, v_packed_ct: &Vec<PolyMatrixRaw>) -> Vec<u8> {
  ...
  for instance in 0..params.instances {
    ...
    let mut rest_rows = packed_ct.submatrix(
      1,
      0,
      packed_ct.rows - 1,
      packed_ct.cols
...
pub fn expand_query<'a>(
  ...
  // Recuerda, la base de datos es representada como un hypercubo `2^V1 x 2(subindice: 1) x ... x 2(subindice: V2)`.

  // _Query expansion_ [pag. 18, SPIRAL]: El servidor expande el texto cifrado de la solicitud en `2^V1` matrices de codificación Regev (para la primera dimensión) y `V2` codificaciones GSW (para las dimensiones siguientes).

  //   1) [pag. 23, SPIRAL DIAPO] Query contains `2^V1` matrix Regev ciphertexts. Indicator for index along first dimension.
  //   2) [pag. 23, SPIRAL DIAPO] Query contains `V2` GSW ciphertexts. Indicator for index along subsequent dimensions.

  // `V1` es el equivalente a `params.db_dim_1`.
  // Entonces `dim0` es lo mismo que `2^V1`.
  let dim0 = 1 << params.db_dim_1;

  // `V2` es equivalente a `params.db_dim_2`.
  // Entonces `further_dims` es lo mismo que `V2`, que representa todas las demàs dimensiones aparte de la primera.
  ...
  // `g` es el logaritmo (de base dos) de `num_bits_to_gen`. Equivalente a `log2(t_gsw * V2 + 2^V1)`.
  // `g` es equivalente a `r` del _Algorithm 1_ de [pag. 42, SPIRAL].
  let g = log2_ceil_usize(num_bits_to_gen);

  let right_expanded = params.t_gsw * further_dims;
  // `stop_round` es el logaritmo (de base dos) de `right_expanded`.
  ...
  if further_dims > 0 {
    coefficient_expansion(
      &mut v, ////////// Aqui se crea la variable `v`
      g,
      stop_round,
      params,
      &v_w_left,
      &v_w_right,
      &v_neg1,
      params.t_gsw * params.db_dim_2, // Eqivalente a `right_expanded`.
      params.t_exp_left,
      params.t_exp_left
    );

    for i in 0..v.len() {
      println!("v[{}] = {:?}", i, v[i].rows);
    }
    ...
    println!("g: {}", g);
    println!("len: {}", v.len());
    println!("right_expanded: {}", right_expanded);

    for i in 0..right_expanded {
      //////// Hay un error en esto: `i in 0..right_expanded`. Mirar pag 19
...
pub fn process_query(
  ...
  // `public_params` de tipo `PublicParameters` es una estructura definida en `client.rs` que tiene un campo llamado `v_packing`.

  // El campo `public_params.v_packing` es una estructura de datos que almacena una secuencia de elementos `PolyMatrixNTT` (definida en `poly.rs`).

  // Al llamar a `as_ref()`, se obtiene una referencia al valor almacenado en ese campo.

  // Entonces, la variable `v_packing` almacena una referencia a un `Vec` de estructuras `PolyMatrixNTT`.
  ...
  if params.expand_queries {
    ...
    v_reg_reoriented = AlignedMemory64::new(
      query.v_buf.as_ref().unwrap().len()
    );
    v_reg_reoriented
      .as_mut_slice()
      .copy_from_slice(query.v_buf.as_ref().unwrap());
    v_folding = query.v_ct
      .as_ref()
      .unwrap()
      .iter()
      .map(|x| x.ntt())
