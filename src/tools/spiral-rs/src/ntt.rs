...
use crate::{ arith::*, number_theory::*, params::model_parameters::* };

pub fn powers_of_primitive_root(
  root: u64,
  modulus: u64,
  poly_len_log2: usize
) -> Vec<u64> {
...
pub fn build_ntt_tables(poly_len: usize, moduli: &[u64]) -> Vec<Vec<Vec<u64>>> {
  ...
  for coeff_mod in 0..moduli.len() {
    ...
    let root = get_minimal_primitive_root(
      2 * (poly_len as u64),
      modulus
    ).unwrap();
    ...
    let scaled_root_powers = scale_powers_u32(
      modulus_as_u32,
      poly_len,
      root_powers.as_slice()
    );
    let mut inv_root_powers = powers_of_primitive_root(
      inv_root,
      modulus,
      poly_len_log2
    );
    ...
    let scaled_inv_root_powers = scale_powers_u32(
      modulus_as_u32,
      poly_len,
      inv_root_powers.as_slice()
    );
...
pub fn ntt_forward(params: &Params, operand_overall: &mut [u64]) {
  ...
  for coeff_mod in 0..params.crt_count {
    ...
    for mm in 0..log_n {
      ...
      for i in 0..m {
        ...
        for j in 0..t {
          ...
          let curr_x: u32 =
            x -
            two_times_modulus_small * ((x >= two_times_modulus_small) as u32);
    ...
    for i in 0..n {
      operand[i] -=
        ((operand[i] >= (two_times_modulus_small as u64)) as u64) *
        (two_times_modulus_small as u64);
      operand[i] -=
        ((operand[i] >= (modulus_small as u64)) as u64) *
        (modulus_small as u64);
    }
  }
}
...
pub fn ntt_inverse(params: &Params, operand_overall: &mut [u64]) {
  for coeff_mod in 0..params.crt_count {
    ...
    for i in 0..n {
      operand[i] -=
        ((operand[i] >= two_times_modulus) as u64) * two_times_modulus;
        ...
        if t < 4 {
          ...
          unsafe {
            for j in (0..t).step_by(4) {
              ...
              let two_times_modulus_vec = _mm256_set1_epi64x(
                two_times_modulus as i64
              );
              ...
              let to_subtract = _mm256_and_si256(
                gt_mask,
              ...
              let eq_mask = _mm256_cmpeq_epi64(
                _mm256_and_si256(t_tmp, and_mask),
                and_mask
              );
              let to_add = _mm256_and_si256(eq_mask, modulus_vec);

              let new_x = _mm256_srli_epi64(
                _mm256_add_epi64(curr_x, to_add),
                1
              );
