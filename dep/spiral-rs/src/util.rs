use crate::{ arith::*, client::Seed, params::model_parameters::*, poly::* };
...
pub fn decompose_index(indices: &mut [usize], index: usize, lengths: &[usize]) {
  // Al recibir `indices` con una referencia mutable (`&mut`) esta funcion tiene la capacidad de modificar el valor real de la variable original.
...
pub fn write_arbitrary_bits(
  ...
  if bit_off_within_word + num_bits <= 64 {
    let idx = word_off * 8;
    let mut cur_val = u64::from_ne_bytes(
      data[idx..idx + 8].try_into().unwrap()
    );
    ...
    let idx = word_off * 8;
    let mut cur_val = u128::from_ne_bytes(
      data[idx..idx + 16].try_into().unwrap()
    );
...
pub fn reorient_reg_ciphertexts(
  params: &Params,
  out: &mut [u64],
  v_reg: &Vec<PolyMatrixNTT>
) {
    ...
    for r in 0..ct_rows {
      for m in 0..ct_cols {
        for z in 0..params.poly_len {
          ...
          let idx_a_out =
            z * (num_reg_expanded * ct_cols * ct_rows) +
            j * (ct_cols * ct_rows) +
            m * ct_rows +
            r;
          let val1 = v_reg[j].data[idx_a_in + z] % params.moduli[0];
          let val2 =
            v_reg[j].data[idx_a_in + params.poly_len + z] % params.moduli[1];
